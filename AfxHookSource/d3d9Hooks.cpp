...
)
{
	...
	AfxHookSource::Gui::On_Direct3DDevice9_Init(parameters->hDeviceWindow, device, parameters->BackBufferWidth, parameters->BackBufferHeight);
	...
	if (g_OldDirect3D9Ex) {
		hasLuid = SUCCEEDED(g_OldDirect3D9Ex->GetAdapterLUID(adapter, &adapterLUID));
	}
	else {
		Direct3DCreate9Ex_t createDirect3Device9 = old_Direct3DCreate9Ex;
		if (nullptr == createDirect3Device9) {
			HMODULE hD3dD9Dll = GetModuleHandleA("d3d9.dll");
			if(nullptr!= hD3dD9Dll) createDirect3Device9 = (Direct3DCreate9Ex_t)GetProcAddress(hD3dD9Dll, "Direct3DCreate9Ex");
		}

		if (createDirect3Device9) {
			if (SUCCEEDED(createDirect3Device9(D3D_SDK_VERSION, &g_OldDirect3D9Ex))) {
				hasLuid = SUCCEEDED(g_OldDirect3D9Ex->GetAdapterLUID(adapter, &adapterLUID));
				g_OldDirect3D9Ex->Release();
				g_OldDirect3D9Ex = nullptr;
			}
		}
	}

	if (hasLuid) {
		JsonDrawingWindowCreated((int)adapterLUID.LowPart, (int)adapterLUID.HighPart, parameters->BackBufferWidth, parameters->BackBufferHeight);
	}
}

void Shared_Direct3DDevice9_Shutdown()
{
	JsonDrawingWindowDestroyed();
		...
		if (AfxInterop::Enabled() && g_OldDirect3D9Ex && pPresentationParameters)
		{
			...
			if (SUCCEEDED(hRet) && ppReturnedDeviceInterface)
			{
				...
				*ppReturnedDeviceInterface = reinterpret_cast<IDirect3DDevice9 *>(&g_NewDirect3DDevice9);
				...
				Shared_Direct3DDevice9_Init(Adapter, pPresentationParameters, g_OldDirect3DDevice9);
			}
			else
				return D3DERR_NOTAVAILABLE;
		}
